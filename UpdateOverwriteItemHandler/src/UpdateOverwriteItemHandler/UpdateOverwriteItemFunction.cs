...
{
    public class UpdateOverwriteItemFunction
    {
        public async System.Threading.Tasks.Task FunctionHandler(RoutedPodioEvent input, ILambdaContext context)
        {
            ...
            using (var _mysql = new MySqlQueryHandler(context))
            {
                ...
                try
                {
                    podioItemId = await _mysql.DeleteAndInsertPodioItem(podioAppId, input.currentItem.ItemId, int.Parse(input.podioEvent.item_revision_id), input.clientId, input.currentEnvironment.environmentId);
                }
